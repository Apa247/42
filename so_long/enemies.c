...
void	move_enemies(t_map *map)
{
	...
	while (map->split_map[++i] && map->frames_enemies == 1)
	{
		...
		while (map->split_map[i][++k])
		{
			...
			if (map->split_map[i][k] == 'N' && map->enemies_copy == 1)
			{
				map->split_map[i][k] = 'T';
				map->enemy_x = k;
				map->enemy_y = i;
		...
		if (map->split_map[i][k] == 'T' || map->split_map[i][k] == 'N')
			break ;
	}
	...
	if (map->copy_n_enemies == 0 && map->enemies_copy == 0 && map->split_map[i][k] == 'N')
		map->enemies_copy = 1;
	if (map->copy_n_enemies == 0 && map->enemies_copy == 1 && map->split_map[i][k] == 'T')
		map->enemies_copy = 0;
	if (map->copy_n_enemies == 0)
		map->copy_n_enemies = map->n_enemies;
	if (map->frames_enemies == 7500)
